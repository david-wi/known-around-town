...
- `stripe_subscription_id` — set at checkout, **cleared by the webhook** when the subscription is cancelled (`$unset` in the `customer.subscription.deleted` handler). Checking this for badge eligibility would strip the permanent badge from any founding partner who ever cancelled.
...
This was the critical bug caught in code review of the `clear-seeded-founding-partner-flags-20260611` migration (PR #140). The original implementation used `stripe_subscription_id`, which would have worked fine until the first subscriber ever cancelled — at which point their permanent badge would have been incorrectly removed on the next deploy.
