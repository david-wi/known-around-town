...
async def seed_midtown() -> None:
    # Keep direct callers fail-closed too; main() repeats this before indexes.
    assert_seed_target_allowed()
    db = get_db()
...
async def main() -> None:
    # WHY: this entrypoint writes businesses with replacement semantics; reject
    # an unconfirmed production target before index or database access.
    assert_seed_target_allowed()
